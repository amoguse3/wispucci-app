...
# ═══════════════════════════════════════════════════════════
# SKILL 1 — GENERARE CURS / MODUL
# ═══════════════════════════════════════════════════════════

COURSE_SYSTEM = """Ești Wispucci, tutor EdTech. Output STRICT JSON, fără text în afara JSON.
Construiești {count} lecții, fiecare cu:
- title: titlu scurt (max 40 caractere)
- body: teorie (max 200 cuvinte, limbaj simplu, 2-3 paragrafe)
- exercises: 2-3 exerciții.
Tipuri de exerciții:
  fill — completează spațiul (blanks: array de răspunsuri corecte)
  choice — întrebare cu opțiuni (options: array, answer: index)
  code — scrie cod (expected: soluția corectă, hint: indiciu)
  match — potrivește (pairs: [[stânga, dreapta], ...])
Răspuns DOAR JSON, zero caractere în plus."""


def _build_course_prompt(subject: str, topic: str, level: int, lesson_count: int = 3) -> str:
    level_labels = {0: "zero absolut", 1: "începător", 2: "mediu", 3: "avansat"}
    lvl = level_labels.get(level, "mediu")
    return (
        f"Subiect:{subject} Topic:{topic} Nivel:{lvl}({level}/3)\n"
        f"Generează {lesson_count} lecții consecutive, de la bază la avansat.\n"
        f"Prima lecție — conceptele fundamentale. Ultima — aplicație practică."
    )
...
async def generate_course(
    ...
    """
    Build a full module with lessons + exercises.
    Returns parsed JSON dict with 'title' and 'lessons' list.
    """
    system = COURSE_SYSTEM.replace("{count}", str(lesson_count))
    user = _build_course_prompt(subject, topic, level, lesson_count)
    raw = await _call_ai(system, user, json_mode=True, max_tokens=2048)
    try:
